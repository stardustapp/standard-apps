import {
  BlobEntry, EnumerationWriter, interpretUrl, StringEntry,
...
const path = rootPath + '/persist/blog/photos';
...
const enumer = new EnumerationWriter(3);
...
for (const photo of output.Children) {
  ...
  const photoPath = `${path}/${encodeURIComponent(photo.Name)}`;
  // console.log(photo);
  ...
  function putString(path: string, value: string) {
    return client.performOp({
      Op: 'store',
      Dest: photoPath+path,
      Input: new StringEntry('', value),
    });
  }
  ...
  if (data.caption !== fullCaption) await putString('/caption', fullCaption);
  if (data.takenAt !== takenAt.toISOString()) await putString('/taken at', takenAt.toISOString());
  if (data.alternative !== altText) await putString('/alternative', altText);
  if (data.fullResUrl !== hotlinkMatch[1]) await putString('/full res url', hotlinkMatch[1]);
  if (data.caption !== fullCaption) await putString('/caption', fullCaption);
  // profile.callApi('store', photoPath+'/dimensions', Skylink.toEntry('dimensions', {
  //   width: ''+dimensions.width,
  //   height: ''+dimensions.height})).wait();
  if (!data.thumbnail && thumbnail) await client.performOp({
    Op: 'store',
    Dest: photoPath+'/thumbnail',
    Input: thumbnail,
  });
}
